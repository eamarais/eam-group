...
def regrid_and_process():
    ...
    tpmid[46] = np.multiply(0.5, (tpedge[46, y, x] + (tpedge[46, y, x] - 0.1)))  # Why 46?
    ...
    lind = np.where((tpmid >= P_MIN) & (tpmid <= P_MAX))[0]
    ...
    twgt = np.exp(np.multiply((-1.0), np.divide((np.square(np.subtract(tpmid[askind], 315.))),
...
def process_file(file_path):
    global tlon, tlat, tgcno2, tcldfr, tcldhgt, tadn, tbxhgt, tpedge, tpause, tgco3, gno2, go3, allcldfr, stratno2, gcldp, gtrueno2, cntloop, y, x, i, maxcnt, cnt
    ...
    for i in range(xdim):
        for j in range(ydim):
            if gno2[i, j, 0] != 0:
                process_grid_square(allcldfr, gcldp, gno2, go3, gtrueno2, i, j, stratno2)


def process_grid_square(allcldfr, gcldp, gno2, go3, gtrueno2, i, j, stratno2):
    global maxcnt, cnt
    ...
    if ((npnts >= 20) & (npnts < 100)):  # <=30:
        ...
        if np.isnan(csval[0]) or np.isnan(csval[1]):
            loss_count[csval[2] - 1] = loss_count[csval[2] - 1] + 1
            pass  # continue
...
def print_data_loss_report():
    global gno2vmr, gcldfr, trueno2, trueo3, gcld, gerr, gcutno2
    # No. of data points:
    ...
    print('(1) Too few points: ', loss_count[0], flush=True)
    print('(2) Low cloud height range: ', loss_count[1], flush=True)
    print('(3) Low cloud height std dev: ', loss_count[2], flush=True)
    print('(4) Large error: ', loss_count[3], flush=True)
    print('(5) Significantly less then zero: ', loss_count[4], flush=True)
    print('(6) Outlier (NO2 > 200 pptv): ', loss_count[5], flush=True)
    print('(7) Non-uniform stratosphere: ', loss_count[6], flush=True)
    print('(8) Successful retrievals: ', cnt, flush=True)
    print('(9) Total possible points: ', (np.sum(loss_count) + cnt), flush=True)
    # Get average:
...
if __name__ == "__main__":
    ...
    loss_count = np.zeros(7)  # TODO: Refactor to dict. These are individual counts for error loss.
    ...
    plot_data()
    out_path = './Data/gc-v12-1-0-ut-no2-' + REGION.lower() + '-jja-' + yrrange + '-' + STR_RES + '-' + prange + '-v4.nc'
